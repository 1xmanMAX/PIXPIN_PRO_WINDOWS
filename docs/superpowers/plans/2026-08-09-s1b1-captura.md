...
- **`#![forbid(unsafe_code)]`** en `pixpin-geom`, `pixpin-model`, `pixpin-store`, `pixpin-ui`, `pixpin-flow`, `pixpin-plugin` y `apps/pixpin`. Si una tarea necesitase `unsafe` en uno de ellos, la frontera de capas se ha roto: **párate y dilo**, no relajes la regla.
...
- **Regla de capas:** L0 `geom, model` · L1 `shell, render, gpu, codec` · L2 `capture, pin, pdf, ocr, record, store` · L3 `ui, flow, plugin` · L4 `pixpin`. Sólo se depende hacia abajo. `apps/pixpin/tests/capas.rs` lo hace cumplir.
...
- [ ] Las tres métricas están anotadas con sus valores reales
